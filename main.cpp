...
public:
    ...
    bool mission_check(int item){
        ...
        if(max_length == mission[0]) mvwprintw(miss, 4, 9, " (V)");
        switch(item){
            case GROWTH:
                tmp = "+ :  "+ to_string(sc_growth);
                mvwprintw(info, 4, 2, to_char(tmp, b));
                if(sc_growth == mission[1]) mvwprintw(miss, 5, 9, "(V)");
                break;
            case POISON:
                tmp = "- :  "+ to_string(sc_poison);
                mvwprintw(info, 5, 2, to_char(tmp, b));
                if(sc_poison == mission[2]) mvwprintw(miss, 6, 9, "(V)");
                break;
            case GATE1:
            case GATE2:
                tmp = "G :  "+ to_string(sc_gate);
                mvwprintw(info, 6, 2, to_char(tmp, b));
                if(sc_gate == mission[3]) mvwprintw(miss, 7, 9, "(V)");
                break;
        }
...
int main() {
    ...
    for(int i = 1; i<=5; i++){
        ...
        if(game.game_over){
            wclear(mission_win);
            mvwprintw(mission_win, 1, 1, "[ Game Over! ]");
            mvwprintw(mission_win, 3, 1, "ReStart?");
            mvwprintw(mission_win, 4, 1, "Y/N");
            wborder(mission_win, '@','@','@','@','@','@','@','@');
            wrefresh(mission_win);
            int b;
            while(true){
            ...
            wclear(mission_win);
            mvwprintw(mission_win, 1, 1, "[ Game Clear! ]");
            mvwprintw(mission_win, 3, 1, "ReStart?");
            mvwprintw(mission_win, 4, 1, "Y/N");
            wborder(mission_win, '@','@','@','@','@','@','@','@');
            wrefresh(mission_win);
